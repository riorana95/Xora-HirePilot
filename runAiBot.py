...
def get_applied_job_ids() -> set[str]:
    from modules.jobs_logger import get_applied_job_ids as _get_ids, PLATFORM_LINKEDIN
    return _get_ids(PLATFORM_LINKEDIN)
...
def failed_job(job_id: str, job_link: str, resume: str, date_listed, error: str, exception: Exception, application_link: str, screenshot_name: str) -> None:
    from modules.jobs_logger import failed_job as _failed, PLATFORM_LINKEDIN
    _failed(PLATFORM_LINKEDIN, job_id, job_link, resume, date_listed, error, exception, application_link, screenshot_name)
...
def submitted_jobs(job_id: str, title: str, company: str, work_location: str, work_style: str, description: str, experience_required: int | Literal['Unknown', 'Error in extraction'], 
    ...
    from modules.jobs_logger import submitted_job, PLATFORM_LINKEDIN
    submitted_job(
        PLATFORM_LINKEDIN, job_id, title, company, work_location, work_style, description,
        experience_required, skills, hr_name, hr_link, resume, reposted, date_listed,
        date_applied, job_link, application_link, questions_list, connect_request,
    )
                    ...
                    if try_xp(driver, ".//button[contains(@class,'jobs-apply-button') and contains(@class, 'artdeco-button--3') and contains(@aria-label, 'Easy')]"):
                        try: 
                            try:
                                ...
                                sleep(10)  # Add 30-second delay before submitting to avoid detection
...
def main() -> None:
    parser = argparse.ArgumentParser(description="LinkedIn Auto Job Applier")
    parser.add_argument("--no-alerts", action="store_true", help="Disable popup alerts (used when launched from dashboard)")
    ...
    try:
        ...
        from modules.open_chrome import init_chrome_session
        ...
        if not os.path.exists(default_resume_path):
            resume_msg = 'Your default resume "{}" is missing! Please update default_resume_path in config/questions.py\n\nOR add a resume at the exact path.\n\nFor now the bot will use your previous LinkedIn upload.'.format(default_resume_path)
            if not skip_alerts:
                pyautogui.alert(text=resume_msg, title="Missing Resume", button="OK")
            else:
                print_lg(resume_msg)
            useNewResume = False
