...
def get_bot_profile_directory() -> str:
    '''Dedicated Chrome profile for the job bot (separate from your daily browser — avoids profile lock crashes).'''
...
def get_default_temp_profile() -> str:
    # Deprecated alias — returns profile directory path only
    return get_bot_profile_directory()


def is_headless() -> bool:
    '''Check if the bot is running in headless/background mode.'''
...
def manual_login_retry(is_logged_in: callable, limit: int = 2) -> None:
    ...
    while not is_logged_in():
        if is_headless():
            print_lg("Not logged in and running in headless mode. Please run once with run_in_background = False to complete login.")
            return
        from pyautogui import alert
