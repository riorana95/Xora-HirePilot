...
def validate_settings() -> None | ValueError | TypeError:
    ...
    check_string(freshness, "freshness")
    check_list(filter_locations, "filter_locations")
    check_list(departments, "departments")
    check_list(salary_ranges, "salary_ranges")
    check_list(company_types, "company_types")
    check_list(role_categories, "role_categories")
    check_list(educations, "educations")
    check_list(posted_by, "posted_by")
    check_list(industries, "industries")
    check_list(top_companies, "top_companies")
...
def validate_search_naukri() -> None | ValueError | TypeError:
    global __validation_file_path
    __validation_file_path = "config/search_naukri.py"

    check_list(search_terms, "search_terms", min_length=1)
    check_list(search_locations, "search_locations", min_length=1)
    check_boolean(randomize_search_order, "randomize_search_order")
    check_int(max_applications_per_search, "max_applications_per_search", 1)
    check_int(experience_min, "experience_min", -1)
    check_int(experience_max, "experience_max", -1)
    check_int(salary_min_lpa, "salary_min_lpa", 0)
    check_int(salary_max_lpa, "salary_max_lpa", 0)
    check_list(work_mode, "work_mode", ["Remote", "Hybrid", "Office"])
    check_int(job_age_days, "job_age_days", 0)
    check_boolean(apply_on_naukri_only, "apply_on_naukri_only")
    check_boolean(pause_after_filters, "pause_after_filters")
    check_boolean(run_non_stop, "run_non_stop")
    check_list(about_company_bad_words, "about_company_bad_words")
    check_list(about_company_good_words, "about_company_good_words")
    check_list(bad_title_words, "bad_title_words")
    check_list(bad_words, "bad_words")
    check_boolean(security_clearance, "security_clearance")
    check_boolean(did_masters, "did_masters")
    check_int(current_experience, "current_experience", -1)
...
def validate_naukri_questions() -> None | ValueError | TypeError:
    global __validation_file_path
    __validation_file_path = "config/naukri_questions.py"
    check_string(willing_to_relocate, "willing_to_relocate", ["Yes", "No"])
    check_string(default_yes_no_answer, "default_yes_no_answer", ["Yes", "No"])
    check_boolean(pause_at_unknown_naukri_question, "pause_at_unknown_naukri_question")
    if not isinstance(custom_naukri_answers, dict):
        raise TypeError('custom_naukri_answers must be a dict in config/naukri_questions.py')


def validate_naukri_config() -> bool | ValueError | TypeError:
    from modules.config_manager import repair_bool_fields_on_disk
    repair_bool_fields_on_disk()
    import importlib
    import config.naukri_questions as nq
    importlib.reload(nq)
    global willing_to_relocate, default_yes_no_answer, pause_at_unknown_naukri_question, custom_naukri_answers
    willing_to_relocate = nq.willing_to_relocate
    default_yes_no_answer = nq.default_yes_no_answer
    pause_at_unknown_naukri_question = nq.pause_at_unknown_naukri_question
    custom_naukri_answers = nq.custom_naukri_answers
    validate_personals()
    validate_questions()
    validate_search_naukri()
    validate_naukri_questions()
    validate_secrets()
    validate_settings()
    return True
